...
SEAlaskacounter <- 1
Arcticcounter <- 1
NBSmatrix <- matrix(nrow = 300, ncol = 20)
SBSmatrix <- matrix(nrow = 300, ncol = 20)
SEAlaskamatrix <- matrix(nrow = 300, ncol = 20)
Arcticmatrix <- matrix(nrow = 300, ncol = 20)

for (i in 1:length(meta$Sample.ID)) {
  for(j in 1:18) {
    if(meta[i,6] == "Northern Bering Sea"){
      NBSmatrix[NBScounter, j] <- as.character(meta[i, j])
      if(j == 18){
        NBScounter <- NBScounter + 1
      }}
      if(meta[i,6] == "Southern Bering Sea"){
        SBSmatrix[SBScounter, j] <- as.character(meta[i, j])
        if(j == 18){
